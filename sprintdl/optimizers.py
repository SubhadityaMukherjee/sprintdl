...
from .helpers import compose, listify
...
class AverageGrad(Stat):
    ...
    def update(self, p, state, mom, **kwargs):
        state["mom_damp"] = tensor(1 - mom if self.dampening else 1.0)
...
def lamb_step(
    p, lr, mom, mom_damp, step, sqr_mom, sqr_damp, grad_avg, sqr_avg, eps, wd, **kwargs
):
    debias1 = debias(mom, mom_damp, step)
    debias2 = debias(sqr_mom, sqr_damp, step)
...
def sgd_step(p, lr, **kwargs):
    p.data.add_(-tensor(lr), p.grad.data)
    return p


def momentum_step(p, lr, grad_avg, **kwargs):
    p.data.add_(-tensor(lr), grad_avg)
    return p
